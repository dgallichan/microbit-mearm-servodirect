...
basic.forever(function () {
    if (!(isIdle)) {
        ...
        servoGrip_target = Math.constrain(servoGrip_target, 100, 153)
        servoBase = Math.constrain(smoothFactor * servoBase + (1 - smoothFactor) * servoBase_target, 45, 135)
        servoLeft = Math.constrain(smoothFactor * servoLeft + (1 - smoothFactor) * servoLeft_target, 35, 155)
        servoRight = Math.constrain(smoothFactor * servoRight + (1 - smoothFactor) * servoRight_target, 45, 160)
        servoGrip = Math.constrain(smoothFactor * servoGrip + (1 - smoothFactor) * servoGrip_target, 100, 153)
